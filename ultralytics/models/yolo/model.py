...
class YOLO(Model):
    ...
    def __init__(self, model: Union[str, Path] = "yolo11n.pt", task: Optional[str] = None, verbose: bool = False):
        ...
        if "-world" in path.stem and path.suffix in {".pt", ".yaml", ".yml"}:  # if YOLOWorld PyTorch model
            if "-moe" in path.stem and path.suffix in {".pt", ".yaml", ".yml"}:
                new_instance = YOLOWorld_MoE(path, verbose=verbose)
    ...
    def task_map(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "detect": {
                ...
                "validator": yolo.detect.DetectionValidator,
                "predictor": yolo.detect.DetectionPredictor,
                "trainer": yolo.world_moe.WorldTrainerMoE,
            }
        }
