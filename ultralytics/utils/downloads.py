...
def safe_download(
    ...
    if "://" not in str(url) and Path(url).is_file():  # URL exists ('://' check required in Windows Python<3.10)
        ...
        for i in range(retry + 1):
            try:
                # subprocess.run(['bash', '/home/wfx/login.sh'], check=True)
                if (curl or i > 0) and curl_installed:  # curl download with retry, continue
                    s = "sS" * (not progress)  # silent
                    r = subprocess.run(["curl", "-#", f"-{s}L", url, "-o", f, "--retry", "5", "-C", "-"]).returncode
                ...
                # NOTE: 原作者残留的本地登录脚本 `/home/wfx/login.sh` 在本机不存在，
                # 触发 retry 时会让整个下载流程直接崩溃，这里去掉。
                time.sleep(10)
