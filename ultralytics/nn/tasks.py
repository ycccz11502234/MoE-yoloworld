...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):
        ...
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            ...
            dummy_outputs = _forward(torch.zeros(1, ch, s, s))
            if len(dummy_outputs) == 2:
                m.stride = torch.tensor([s / x.shape[-2] for x in dummy_outputs[0]])  # forward
            else:
                m.stride = torch.tensor([s / x.shape[-2] for x in dummy_outputs])  # forward
...
class WorldModel_MoE(DetectionModel):
    """
    YOLOv8 World Model with Mixture of Experts support.

    This class implements the YOLOv8 World model for open-vocabulary object detection with MoE architecture,
    supporting text-based class specification and CLIP model integration for zero-shot detection capabilities.
    ...
    Methods:
        __init__: Initialize YOLOv8 world model with MoE.
        set_classes: Set classes for offline inference.
        get_text_pe: Get text positional embeddings.
        predict: Perform forward pass with text features.
        loss: Compute loss with text features.
    ...
    Examples:
        Initialize a world model with MoE
        >>> model = WorldModel_MoE("yolov8s-world.yaml", ch=3, nc=80)
        >>> model.set_classes(["person", "car", "bicycle"])
        >>> results = model.predict(image_tensor)
    """
    ...
    def predict(self, x, profile=False, visualize=False, txt_feats=None, augment=False, embed=None):
        ...
        embed = frozenset(embed) if embed is not None else {-1}
        max_idx = max(embed)
        moe_gate_logits_list = []  # ← local list, no global state!
        moe_topk_idx_list = []      # for loss-free bias updates (final variant)
        ...
        for m in self.model:  # except the head part
            ...
            if isinstance(m, C2fAttn):
                x = m(x, txt_feats)
            elif isinstance(m, MoEGate_attn):
                gate_logits = m(x, txt_feats)  # (B, num_experts)
                expert_indices = torch.argmax(gate_logits, dim=-1)
            elif isinstance(m, MoEGate):
                gate_logits = m(x)  # (B, num_experts)
                expert_indices = torch.argmax(gate_logits, dim=-1)
            elif isinstance(m, WorldDetect):
                x = m(x, ori_txt_feats)
            elif isinstance(m, ImagePoolingAttn):
                txt_feats = m(x, txt_feats)
            elif isinstance(m, MoE_C2f):
                x = m(x, expert_indices)
            elif isinstance(m, MoE_C2f_gate):
                x, gate_logits = m(x)
                if self.training:
                    moe_gate_logits_list.append(gate_logits)
            elif isinstance(m, MoE_C2f_final):
                x, glogits_list, tidx_list = m(x, txt_feats)
                if self.training:
                    moe_gate_logits_list.extend(glogits_list)
                    moe_topk_idx_list.extend(tidx_list)
            elif isinstance(m, MoE_C2f_final_conv):
                x, glogits_list, tidx_list = m(x, txt_feats)
                if self.training:
                    moe_gate_logits_list.extend(glogits_list)
                    moe_topk_idx_list.extend(tidx_list)
            ...
            if visualize:
                feature_visualization(x, m.type, m.i, save_dir=visualize)
            if m.i in embed:
                if isinstance(m, MoEGate) or isinstance(m, MoEGate_attn) or isinstance(m, MoE_C2f_gate):
                    return gate_logits
                else:
                    embeddings.append(torch.nn.functional.adaptive_avg_pool2d(x, (1, 1)).squeeze(-1).squeeze(-1))  # flatten
                    if m.i == max_idx:
                        return torch.unbind(torch.cat(embeddings, 1), dim=0)
        if self.training:
            # Stash for loss-free bias update by the trainer
            self._last_moe_topk = moe_topk_idx_list
            return x, moe_gate_logits_list
        else:
            return x

    def init_criterion(self):
        """Initialize the loss criterion for the DetectionModel."""
        return v8_world_DetectionLoss(self)
    ...
    def loss(self, batch, preds=None):
        ...
        Args:
            ...
            preds (torch.Tensor | List[torch.Tensor], optional): Predictions.
        """
        moe_gate_logits_list = []
        if not hasattr(self, "criterion"):
            self.criterion = self.init_criterion()

        if preds is None:
            preds, moe_gate_logits_list = self.forward(batch["img"], txt_feats=batch["txt_feats"])
            
        return self.criterion(preds, batch, moe_gate_logits_list)
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            c1 = ch[f]
            args = [c1, *args[1:]]
        elif m is MoEGate_attn:
            # set 1) expert_num 2) embed channels and 3) num heads
            c1 = ch[f]
            args[2] = make_divisible(min(args[2], max_channels // 2) * width, 8)
            args[3] = int(max(round(min(args[3], max_channels // 2 // 32)) * width, 1) if args[2] > 1 else args[2])
            args = [c1, *args[1:]]
        else:
            c2 = ch[f]
